...
class EliminaMarcacaoFragment: Fragment() {
    ...
    private fun eliminaMarcacao() {
        ...
        if(registos != 1){
            Toast.makeText(requireContext(),"erro ao eliminar marcação", Toast.LENGTH_LONG).show()
            return
        }
        Toast.makeText(requireContext(),"Marcação eliminada com sucesso", Toast.LENGTH_LONG).show()
        navegaListaVacinas()
    }
